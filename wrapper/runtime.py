...
# Same ceiling as llm-init's wait_for_sentinel. The load watchdog starts after
# this returns: its 1800s budget is for reading the snapshot, not for download.
...
class Runtime:
    ...
    def _hub_spec(self):
        # Keep --include / --exclude so a partial llm-init fetch is not judged as
        # a missing snapshot (FireRed, Voxtral GGUF, VoxCPM2).
    ...
    def serve(
        ...
        *,
        ...
        def boot():
            # Arm after the snapshot is on disk so download time does not eat the
            # load deadline (watchdog assumes cache-local weights).
