...
# OpenVINO batch probe / contract tests still use this name. Align with main's ceiling
# when the operator wrote one; measurement-on with no ceiling is still a batch.
...
def _ov_generate_many(clips, language=None):
    ...
    try:
        ...
        _state["asr"] = None
        _load_ov()
        return [_ov_result_text(_ov_generate(c, language=language)) for c in clips]
...
def _offline_transcribe_many(clips, language=None, context=""):
    # OpenVINO GenAI generate() takes one waveform on stock wheels; the patched
    # ov image accepts a list so the group is one generate(). context/glossary
    # is CUDA-only. The caller's grouping, halving and cancel bound still run.
    if _is_ov():
        texts = _ov_generate_many(clips, language=language)
        return [(t, language or "") for t in texts]
