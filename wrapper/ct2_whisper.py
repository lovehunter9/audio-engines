# Turn a Systran / faster-whisper CT2 snapshot into a transformers Whisper dir using only files on disk.
import json
import logging
import os
import shutil
...
def is_whisper_ir(path):
    # WhisperPipeline needs beam_idx; a stateless ASR export 500s with that port missing.
...
def _dump_ct2_state_dict(src):
    # Parse model.bin in Python; the Intel CT2 wheel SIGSEGVs inside ctranslate2.models.Whisper.
...
def _to_hf_names(ct2_state, config):
    # Inverse of CTranslate2 WhisperLoader: self-attn is fused QKV + out; cross-attn is Q / fused KV / out.
    ...
    def attn(prefix, dest, kind):
        # kind: self (2 linears) or cross (3); a 4-way dump is leftover so the tiny unit dump still maps.
...
# openai/whisper-large-v3; GenAI WhisperPipeline::generate 500s multilingual models when this map is missing.
_WHISPER_LANG_TO_ID = {
