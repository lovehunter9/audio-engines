...
# Hash deps.Dockerfile plus anything it COPY's. Sorted concat so the tag
# moves when a copied file changes, not only the Dockerfile.
hash_inputs="$file"
for f in "$dir"/probe_*.py "$dir"/patches/*.py \
