...
class TtsOvCausalHelpersTest(unittest.TestCase):
    ...
    def test_breeze_install_does_not_swallow_export_failure(self):
        ...
        self.assertIn("_install_firered_flow(", ov_src)
        self.assertIn("_install_firered_redae_cache(", ov_src)
        self.assertIn("_install_firered_decode_log(", ov_src)
        self.assertIn("PERFORMANCE_HINT", ov_src)
        self.assertIn("device-KV decode", ov_src)
        flow = inspect.getsource(firered._install_firered_flow)
        self.assertIn("create_infer_request", flow)
        self.assertIn("core._flow_one_step", flow)
        self.assertIn('times["dit"]', flow)
        self.assertIn("torch.randn", flow)
