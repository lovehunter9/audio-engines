# Pyannote / Silero / SpeechBrain: own image; pin 4.x onto the torch we just installed, then strip unused nvidia-*.
ARG TARGETARCH
...
RUN set -eux; \
    ...
    # Do not pip-uninstall 4.x extras: import still needs pandas, otel, and optuna.
