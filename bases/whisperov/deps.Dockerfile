# audio-whisper-ov deps. No Qwen ASR, no GenAI rebuild, no CUDA; Intel GPU is amd64 only.
FROM ubuntu:24.04
...
# Pip wheels only. ctranslate2 reads Systran model.bin; no torch — converter writes safetensors from numpy.
